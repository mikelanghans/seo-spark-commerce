...
export const ShopifySettings = ({ userId, organizationId }: Props) => {
  ...
  const GLOBAL_SHOPIFY_CLIENT_ID = import.meta.env.VITE_SHOPIFY_CLIENT_ID as string | undefined;
  const usingGlobalApp = Boolean(GLOBAL_SHOPIFY_CLIENT_ID) && !existing?.client_id;
  ...
  // Full-page redirect to Shopify's OAuth consent screen — same pattern as
  // the Etsy connect flow. Shopify redirects back to shopify-oauth-callback,
  // which redirects the browser straight back here with a ?shopify_oauth=
  // success/error param, picked up by the "Check URL for OAuth callback
  // params" effect above. No popup, no window.opener/postMessage dependency,
  // no Safari COOP issues — this tab just navigates away and comes back.
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    if (!usingGlobalApp && !clientId.trim()) {
      toast.error("Please enter your Shopify app Client ID");
      return;
    }
    if (!usingGlobalApp && !clientSecret.trim() && !existing?.has_credentials) {
      toast.error("Please enter your Shopify app Client Secret");
      return;
    }

    const domain = storeDomain.trim().replace(/^https?:\/\//, "").replace(/\/$/, "");
    const effectiveClientId = usingGlobalApp ? GLOBAL_SHOPIFY_CLIENT_ID! : clientId.trim();
  ...
  const handleReauthorize = () => {
    ...
    const installUrl = buildInstallUrl(existing.store_domain, effectiveClientId);
    ...
    <div className="space-y-4">
      ...
      {!existing?.has_token && (
        <form onSubmit={handleConnect} className="space-y-4">
          {existing && existing.has_credentials && (
            <div className="space-y-2 rounded-lg bg-amber-500/10 px-3 py-2 text-sm text-amber-600">
              <div className="flex items-center gap-2">
                ...
                {pendingAuthUrl ? (
                  <>Almost there — click <span className="font-medium">"Open Shopify Authorization"</span> below to finish connecting <span className="font-medium">{existing.store_domain}</span>.</>
                ) : usingGlobalApp ? (
                  <>Store domain saved for <span className="font-medium">{existing.store_domain}</span>, but authorization is incomplete. Click <span className="font-medium">"Install &amp; Connect"</span> below to regenerate the authorization link.</>
                ) : (
                  <>Credentials saved for <span className="font-medium">{existing.store_domain}</span>, but authorization is incomplete. Re-enter your Client Secret and click <span className="font-medium">"Install &amp; Connect"</span> below to regenerate the authorization link.</>
                )}
              </div>
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={existing?.has_credentials ? "••••••••  (saved — enter new value to change)" : "e.g. shpss_abc123..."}
                  className="font-mono text-sm"
          ...
          <Button type="submit" variant={pendingAuthUrl ? "outline" : "default"} disabled={saving} className="gap-2">
      ...
      {existing && (
        <div className="space-y-3">
          ...
          {pendingAuthUrl && (
            <div className="space-y-2">
              <div className="flex gap-2 flex-wrap">
                <Button type="button" className="gap-2" onClick={handleAuthorizationLinkClick}>
                  <RefreshCw className="h-4 w-4" />
              ...
              <p className="text-xs text-muted-foreground">
                This takes you to Shopify to authorize, then brings you back here automatically.
              </p>
            </div>
          )}
