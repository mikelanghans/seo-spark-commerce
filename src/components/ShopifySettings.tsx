...
export const ShopifySettings = ({ userId, organizationId }: Props) => {
  ...
  const handleAuthorizationLinkClick = () => {
    ...
    const popup = window.open(pendingAuthUrl, "shopify-oauth", "popup,width=600,height=720");
    if (!popup) {
      ...
      toast.error("Popup blocked. Use 'Copy URL' below and open it in a new tab manually.");
    }
  };
          ...
          {pendingAuthUrl && (
            <div className="space-y-2">
              <div className="flex gap-2 flex-wrap">
                <Button type="button" variant="outline" className="gap-2" onClick={handleAuthorizationLinkClick}>
                  <RefreshCw className="h-4 w-4" />
