...
- Bullet points must describe the "${normalizedProduct.category}" — NOT attributes of the original source format. ${(excludedSections as string[] | undefined)?.includes("materials") || (excludedSections as string[] | undefined)?.includes("care") ? `Since materials/care details are displayed elsewhere on the storefront, bullets must focus on the DESIGN, theme, mood, lifestyle moments, gifting occasions, and brand voice — NOT fit, fabric, cotton, garment-dye, sizing, wash, or care.` : `Cover relevant attributes (fit, fabric, print quality, sizing for apparel; capacity, material for drinkware; etc.).`}
