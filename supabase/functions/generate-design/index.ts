...
function buildPrompt(
  ...
  const noExtraTextRule = `\n\n🚫 ABSOLUTELY NO EXTRA TEXT: The design must contain ONLY the exact text provided above in "TEXT TO FEATURE". Do NOT add any additional words, taglines, slogans, brand names, dates, or decorative text. If the message already includes an attribution (like "— the universe"), that is part of the provided text. Do NOT invent or add anything else.\n\n🎨 FIGURE GUIDELINES: Abstract celestial figures are ENCOURAGED — think cosmic silhouettes, ethereal forms, celestial entities, star-beings, nebula shapes, moon phases, cosmic eye motifs. These are messages FROM the universe, so the visual language should feel cosmic, mystical, and transcendent. Do NOT include realistic human figures, astronauts in spacesuits, detailed faces, or anatomically specific body parts. Keep figures abstract, ethereal, and otherworldly — like a cosmic messenger, not a person.`;
