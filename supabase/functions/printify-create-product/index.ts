...
serve(async (req) => {
  ...
  try {
    ...
    if (organizationId) {
      ...
      const { data: roleData } = await tokenLookup.rpc("get_org_role", { _user_id: user.id, _org_id: organizationId });
      if (!roleData) {
      ...
      const { data: secrets } = await tokenLookup
        ...
        .single();
      if (secrets?.printify_api_token) printifyToken = await decryptOrPassthrough(secrets.printify_api_token, Deno.env.get("ENCRYPTION_KEY")!);
    }
    ...
    if (publish && createdProduct.id) {
      ...
      if (!publishRes.ok) {
        ...
        // Return immediately after publish; Shopify linking is asynchronous and is
        // recovered by the client-side poll to avoid edge function gateway timeouts.
        syncedShopifyProductId = await getPrintifyExternalShopifyId(printifyToken, effectiveShopId, createdProduct.id);
        if (syncedShopifyProductId && productId) {
          await adminClient.from("products").update({ shopify_product_id: syncedShopifyProductId }).eq("id", productId);
          console.log(`Saved immediately available Printify-synced Shopify product ID: ${syncedShopifyProductId}`);
        } else {
          console.log("External Shopify ID not ready yet — client will poll Printify for the link.");
        }
      }
    }
